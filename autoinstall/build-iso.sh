...
echo "  Größe:     $ISO_SIZE"
echo ""
echo "  ── Rescue-USB erstellen (empfohlen) ──────────────────────────────────────"
echo ""
echo "  Modus A — Neuer USB-Stick (ISO + Backup-Partition):"
echo "    bash create-rescue-usb.sh --iso $OUTPUT_ISO --device /dev/sdX"
echo "    [--copy-from /mnt/backup-usb]    # bestehende Backups übernehmen"
echo ""
echo "  Modus B — Bestehender Ventoy-Stick (empfohlen, keine Repartitionierung):"
echo "    bash create-rescue-usb.sh --ventoy --device /dev/sdX --iso $OUTPUT_ISO"
echo "    [--copy-from /mnt/backup-usb]"
echo ""
echo "  ── Oder manuell auf USB schreiben (nur Installer, kein Backup) ──────────"
echo ""
echo "    dd if=$OUTPUT_ISO of=/dev/sdX bs=4M status=progress"
echo "    Balena Etcher / Rufus (Windows)"
echo ""
echo "  ── Ablauf nach dem Booten ────────────────────────────────────────────────"
echo ""
echo "    1. USB booten → Proxmox installiert (~3 Min)"
echo "    2. First-Boot Menü erscheint:"
echo "       [1] Fresh-Install   [2] Disaster Recovery"
echo "    3. [2] + Enter → alle LXCs/VMs aus Backup wiederhergestellt"
echo ""
